...
func (r *AgentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			r.Log.Infof("XXX: in if k8serrors.IsNotFound ")
...
func (r *AgentReconciler) deregisterHostIfNeeded(ctx context.Context, key types.NamespacedName) (ctrl.Result, error) {
	...
	r.Log.Infof("XXX: start deregisterHostIfNeeded ")
	h, err := r.Installer.GetHostByKubeKey(key)

	if errors.Is(err, gorm.ErrRecordNotFound) {
		r.Log.Infof("XXX: deregisterHostIfNeeded: in gorm.ErrRecordNotFound")
	...
	r.Log.Infof("XXX: deregisterHostIfNeeded: got host: id %s , clusterID %s , kname %s , knamespace %s",
		h.ID, h.ClusterID, h.KubeKeyName, h.KubeKeyNamespace)
	r.Log.Infof("XXX: deregisterHostIfNeeded: before DeregisterHostInternal")
	if err = r.Installer.DeregisterHostInternal(ctx, installer.DeregisterHostParams{
		ClusterID: h.ClusterID,
		HostID:    *h.ID,
	}); err != nil {
		r.Log.Infof("XXX: deregisterHostIfNeeded: after DeregisterHostInternal, got err: %s", err.Error())
